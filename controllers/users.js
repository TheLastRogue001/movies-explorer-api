...
const generateToken = (payload) => jwt.sign(payload, NODE_ENV === 'production' ? JWT_SECRET : 'secret-key', {
  expiresIn: '7d',
});
  ...
  User.findById(userId)
    ...
    .catch((err) => {
      if (err.name === 'CastError') {
        return next(
          new BadRequestError('Переданы некорректные данные в поле _id'),
        );
      }
      return next(err);
    });
...
const createUser = (req, res, next) => {
  ...
  bcrypt
    ...
    .catch((err) => {
      ...
      if (err.code === DB_DUBLICATE_ERROR_CODE) {
        return next(
